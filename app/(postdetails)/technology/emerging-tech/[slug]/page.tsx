...
// no Suspense required for view count
  ...
  try {
    ...
    if (!post) {
      return {
        ...
        description:
          "The requested emerging technology article could not be found.",
        robots: "noindex, nofollow",
      };
    }

    const canonicalUrl = `https://www.geokhub.com/technology/emerging-tech/${slug}`;
...
function getPostUrl(post: any): string {
  ...
  // Check each category for "emerging-tech" or related categories
...
export default async function EmergingTechDetailPage({
  ...
  try {
    ...
    const isEmergingTechPost = post.categories?.some((cat) => {
