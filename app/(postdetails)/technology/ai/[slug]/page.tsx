  ...
  try {
    ...
    const description =
      ...
      (post.body
        ...
        : "Latest AI and machine learning insights on GeokHub");
    ...
    return {
      ...
      openGraph: {
        ...
        authors: [post.author?.name || "GeokHub AI Team"],
...
function getAICategory(aiType: string): string {
  if (!aiType) return "General AI";
  ...
  const aiLower = aiType.toLowerCase();
  for (const [key, value] of Object.entries(aiMap)) {
    if (aiLower.includes(key)) return value;
  }
...
export default async function AIDetailPage({
  ...
  try {
    ...
    const [aiNews, relatedAINews, trendingAI] = await Promise.all([
      ...
      client.fetch(
        `*[_type == "post" && count((categories[]->slug.current)[@ in ["ai", "artificial-intelligence"]]) > 0] | order(views desc)[0...5] {
          _id,
          title,
          "slug": slug.current,
          publishedAt,
    ...
    const aiNewsWithUrls =
      aiNews?.map((news: any) => ({
        ...news,
        url: getPostUrl(news),
      })) || [];

    const relatedAINewsWithUrls =
      relatedAINews?.map((news: any) => ({
        ...news,
        url: getPostUrl(news),
      })) || [];

    const trendingAIWithUrls =
      trendingAI?.map((news: any) => ({
        ...news,
        url: getPostUrl(news),
      })) || [];
    ...
    const aiCategory = getAICategory(post.technologyType);
    ...
    const jsonLd = {
      ...
      "@type": "TechArticle",
      headline: post.title || "AI Insights",
      description: post.excerpt || plainTextContent.substring(0, 160),
      author: {
        "@type": "Person",
        name: post.author?.name || "GeokHub AI Team",
      ...
      publisher: {
        ...
        url: "https://www.geokhub.com/technology/ai",
        logo: {
          "@type": "ImageObject",
          url: `${baseUrl}/icons/geokhub-ai.png`,
        },
      },
      ...
      <>
        ...
        <div className="min-h-screen bg-background dark:bg-background-dark">
              ...
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm font-medium">
                    AI Insights • {aiCategory.toUpperCase()}
                  </span>
                </div>
          ...
          <div className="max-w-7xl mx-auto px-0 md:px-4 lg:px-6 py-8">
                  ...
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                    <div>
                      <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">
                        Share this AI Insight
                      </h3>
                      <p className="text-gray-600 dark:text-gray-400">
                        Help the AI community stay updated
                      </p>
                    </div>
                  ...
                  <div className="bg-card rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                    <div className="bg-gradient-to-r from-purple-600 to-indigo-600 text-white p-5">
                      <div className="flex items-center gap-3">
                        ...
                        <h2 className="text-xl font-bold">Trending in AI</h2>
                      </div>
                    </div>
                    <div className="p-2">
                      {trendingAIWithUrls?.length > 0 ? (
                        trendingAIWithUrls
                          .slice(1, 4)
            ...
            {relatedAINewsWithUrls && relatedAINewsWithUrls.length > 0 && (
