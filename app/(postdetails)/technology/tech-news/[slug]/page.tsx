  ...
  try {
    ...
    const description =
      ...
      (post.body
        ...
        : "Latest technology news and insights on GeokHub");
    ...
    return {
      ...
      openGraph: {
        ...
        authors: [post.author?.name || "GeokHub Tech News Team"],
        images: [{ url: imageUrl, width: 1200, height: 630, alt: post.title }],
        siteName: "GeokHub Tech News",
        ...(post.technologyType && { section: post.technologyType }),
      },
      twitter: {
...
function getTechCategory(techType: string): string {
  if (!techType) return "General Tech";
...
export default async function TechDetailPage({
  ...
  try {
    ...
    // ========== FETCH RELATED DATA ==========
    const [techNews, relatedTechNews, trendingTech] = await Promise.all([
      client.fetch(
        BLOG_BY_CATEGORY_SLUG,
        { slug: "tech-news" },
        { timeout: 10000 },
    ...
    const techNewsWithUrls =
      techNews?.map((news: any) => ({
        ...news,
        url: getPostUrl(news),
      })) || [];

    const relatedTechNewsWithUrls =
      relatedTechNews?.map((news: any) => ({
        ...news,
        url: getPostUrl(news),
      })) || [];

    const trendingTechWithUrls =
      trendingTech?.map((news: any) => ({
        ...news,
        url: getPostUrl(news),
      })) || [];
    ...
    const techCategory = getTechCategory(post.technologyType);
    ...
    const jsonLd = {
      ...
      "@type": "TechArticle",
      headline: post.title || "Tech News",
      description: post.excerpt || plainTextContent.substring(0, 160),
      author: {
        "@type": "Person",
        name: post.author?.name || "GeokHub Tech News Team",
      ...
      publisher: {
        ...
        url: "https://www.geokhub.com/technology/tech-news",
        logo: {
          "@type": "ImageObject",
          url: `${baseUrl}/icons/geokhub-tech.png`,
        },
      },
      articleSection: post.categories?.[0]?.title || "Technology News",
      ...(post.breakingNews && { dateline: "TECH BREAKING" }),
    ...
    return (
      <>
        ...
        <div className="min-h-screen bg-background dark:bg-background-dark">
          {/* Tech Navigation Indicator */}
          <div className="bg-gradient-to-r from-purple-900 via-indigo-800 to-blue-900 text-white">
            <div className="max-w-7xl mx-auto px-4 py-2">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Cpu className="h-5 w-5" />
                  <span className="text-sm font-medium">
                    TECH NEWS • {techCategory.toUpperCase()}
                  </span>
                </div>
          ...
          <div className="md:max-w-7xl md:mx-auto px-0 md:px-4 lg:px-6 py-8">
            <div className="flex flex-col lg:flex-row gap-8">
              ...
              <div className="hidden lg:block lg:w-20 flex-shrink-0">
                <div className="sticky top-32 space-y-6 z-30">
                  ...
                  {/* Tech Category Badge */}
                  <div className="bg-gradient-to-br from-purple-600 to-indigo-700 rounded-2xl p-4 text-white text-center">
                    <Network className="h-6 w-6 mx-auto mb-2" />
                    <div className="text-sm font-bold">
                      {techCategory.split(" ")[0]}
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex-1 max-w-4xl mx-auto">
                ...
                {/* Company & Tech Stack */}
                {(post.company || post.technologyType) && (
                  <div className="mb-6">
                    <div className="flex items-center gap-4 flex-wrap">
                      ...
                      {post.technologyType && (
                        <div className="flex items-center gap-2 bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 px-4 py-2 rounded-xl">
                          <Code className="h-4 w-4" />
                          <span className="font-medium">{techCategory}</span>
                        </div>
                      )}
                      {post.complexity && (
                        <div className="ml-auto flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400">
                          <ImCogs className="h-4 w-4" />
                ...
                {/* Article Content */}
                <article className="max-w-7xl mx-auto px-4 lg:px-0 mb-12">
                  <div className="prose prose-lg md:prose-xl dark:prose-invert max-w-none">
                    <div
                      ...
                      prose-headings:border-purple-600
                      prose-headings:pl-4
                      ...
                      prose-a:dark:text-purple-400
                      prose-a:font-medium
                      prose-a:underline
                      prose-code:bg-gray-100
                      prose-code:dark:bg-gray-800
                      prose-code:text-purple-600
                      prose-code:dark:text-purple-400
                      prose-code:p-1
                      prose-code:rounded
                      prose-code:text-sm
                      ...
                      prose-blockquote:dark:bg-purple-900/20
                      ...
                      dark:first-letter:text-purple-400
                    ...
                    <div className="flex flex-wrap gap-3">
                      {post.keywords.map((tag: string, index: number) => (
                        <Link
                          ...
                          className="px-4 py-2 bg-gradient-to-r from-purple-50 to-indigo-50 dark:from-purple-900/30 dark:to-indigo-900/30 text-purple-700 dark:text-purple-300 hover:from-purple-100 hover:to-indigo-100 dark:hover:from-purple-800/50 dark:hover:to-indigo-800/50 rounded-full text-sm font-medium transition-all duration-200 border border-purple-100 dark:border-purple-800"
                        >
                          #{tag}
                        </Link>
                      ))}
              ...
              <aside className="lg:w-80">
                <div className="sticky top-32 space-y-8">
                  ...
                  <div className="bg-card rounded-2xl shadow-xl border border-gray-100 dark:border-gray-700 overflow-hidden">
                    ...
                    <div className="p-2">
                      {trendingTechWithUrls?.length > 0 ? (
                        trendingTechWithUrls
                          .slice(1, 4)
                          .map((newsItem: any, index: number) => {
                            ...
                            return (
                              ...
                              >
                                <div className="flex gap-3 p-2 hover:bg-purple-50 dark:hover:bg-gray-700 rounded-xl transition-all duration-200">
                                  <div className="flex-shrink-0">
                                    <div className="w-8 h-8 flex items-center justify-center bg-gradient-to-br from-purple-500 to-indigo-600 text-white rounded-lg font-bold text-sm">
                                      {index + 1}
                                    </div>
                                  </div>
                                  <div className="flex-1 min-w-0">
                                    <h3 className="font-semibold text-gray-900 dark:text-white group-hover:text-purple-600 dark:group-hover:text-purple-400 line-clamp-2 text-sm">
                                      {newsItem.title}
                                    </h3>
            ...
            {/* Related Tech News Section */}
            {relatedTechNewsWithUrls && relatedTechNewsWithUrls.length > 0 && (
              <section className="mt-20 max-w-7xl mx-auto px-4 md:px-0">
                ...
                <MasonryGrid posts={relatedTechNewsWithUrls.slice(4, 12)} />
                <div className="text-center mt-12">
                  <Link
                    href="/technology/tech-news"
                    className="inline-flex items-center gap-2 px-8 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 hover:from-purple-700 hover:to-indigo-700 text-white rounded-full font-semibold transition-all duration-200 shadow-lg hover:shadow-xl"
                  >
                    Explore All Tech News
    ...
    console.error("Tech news page error:", error);
    notFound();
  }
}
